@extends('layouts.app')

@section('title', $post->meta_title ?: $post->title)

@section('content')
  <div class="col-span-12 lg:col-span-8">
    <nav class="mb-6">
      <a href="{{ route('home') }}" class="text-blue-600 hover:text-blue-800 text-sm">
        ← Volver al blog
      </a>
    </nav>

    <article class="bg-white rounded-2xl border overflow-hidden">
      @if($post->image_url)
        <div class="aspect-video overflow-hidden">
          <img src="{{ $post->image_url }}" 
               class="w-full h-full object-cover" 
               alt="{{ $post->title }}">
        </div>
      @endif

      <div class="p-6 lg:p-8">
        <header class="mb-6">
          <div class="flex items-center justify-between mb-4">
            @if($post->type === 'educational')
              <span class="bg-green-100 text-green-800 text-sm px-3 py-1 rounded-full font-medium">
                Educativo
              </span>
            @else
              <span class="bg-blue-100 text-blue-800 text-sm px-3 py-1 rounded-full font-medium">
                Noticia
              </span>
            @endif
            
            @if($post->tags && count($post->tags) > 0)
              <div class="flex flex-wrap gap-1">
                @foreach(array_slice($post->tags, 0, 4) as $tag)
                  <span class="bg-gray-100 text-gray-700 text-xs px-2 py-1 rounded-full">
                    {{ $tag }}
                  </span>
                @endforeach
              </div>
            @endif
          </div>

          <h1 class="text-2xl lg:text-3xl font-bold text-gray-900 leading-tight mb-4">
            {{ $post->title }}
          </h1>

          <div class="flex items-center text-sm text-gray-600 space-x-4">
            <span>{{ $post->source ?? 'Panorama Ingeniería IA' }}</span>
            <span>•</span>
            <time datetime="{{ $post->published_at?->toISOString() }}">
              {{ $post->published_at?->translatedFormat('d \d\e F \d\e Y \a \l\a\s H:i') }}
            </time>
            @if($post->fetched_at)
              <span>•</span>
              <span>Procesado {{ $post->fetched_at->diffForHumans() }}</span>
            @endif
          </div>
        </header>

        @if($post->excerpt)
          <div class="bg-gray-50 border-l-4 border-blue-500 p-4 mb-6 rounded-r-lg">
            <p class="text-gray-700 font-medium leading-relaxed">
              {{ $post->excerpt }}
            </p>
          </div>
        @endif

        <div class="prose prose-lg max-w-none">
          {!! $post->body !!}
        </div>

        @if($post->source_url)
          <div class="mt-8 p-4 bg-blue-50 border border-blue-200 rounded-lg">
            <p class="text-sm text-blue-800 mb-2">
              <strong>Fuente original:</strong>
            </p>
            <a href="{{ $post->source_url }}" 
               target="_blank" 
               rel="noopener noreferrer"
               class="text-blue-600 hover:text-blue-800 text-sm break-all underline">
              {{ $post->source_url }}
              <svg class="inline w-3 h-3 ml-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14"></path>
              </svg>
            </a>
          </div>
        @endif
      </div>
    </article>

    <!-- Navegación entre posts -->
    <div class="mt-6 grid grid-cols-1 md:grid-cols-2 gap-4">
      @php
        $prevPost = \App\Models\Post::published()
          ->where('published_at', '<', $post->published_at)
          ->orderByDesc('published_at')
          ->first();
        $nextPost = \App\Models\Post::published()
          ->where('published_at', '>', $post->published_at)
          ->orderBy('published_at')
          ->first();
      @endphp

      @if($prevPost)
        <a href="{{ route('post.show', $prevPost->slug) }}" 
           class="bg-white border rounded-lg p-4 hover:bg-gray-50 transition-colors">
          <div class="text-sm text-gray-600 mb-1">← Anterior</div>
          <div class="font-medium text-gray-900">{{ Str::limit($prevPost->title, 60) }}</div>
        </a>
      @endif

      @if($nextPost)
        <a href="{{ route('post.show', $nextPost->slug) }}" 
           class="bg-white border rounded-lg p-4 hover:bg-gray-50 transition-colors md:text-right">
          <div class="text-sm text-gray-600 mb-1">Siguiente →</div>
          <div class="font-medium text-gray-900">{{ Str::limit($nextPost->title, 60) }}</div>
        </a>
      @endif
    </div>
  </div>

  <!-- Sidebar -->
  <aside class="col-span-12 lg:col-span-4 space-y-6">
    <div class="bg-white rounded-2xl border p-6">
      <h3 class="font-bold text-lg mb-4">Posts Relacionados</h3>
      @php
        $relatedPosts = \App\Models\Post::published()
          ->where('id', '!=', $post->id)
          ->when($post->tags, function($query) use ($post) {
            return $query->whereJsonContains('tags', $post->tags[0] ?? 'movilidad');
          })
          ->orderByDesc('published_at')
          ->take(3)
          ->get();
      @endphp

      @forelse($relatedPosts as $related)
        <article class="mb-4 pb-4 @if(!$loop->last) border-b @endif">
          <h4 class="font-medium mb-2">
            <a href="{{ route('post.show', $related->slug) }}" 
               class="text-gray-900 hover:text-blue-600 transition-colors">
              {{ Str::limit($related->title, 70) }}
            </a>
          </h4>
          <div class="text-xs text-gray-600">
            {{ $related->published_at?->diffForHumans() }}
          </div>
        </article>
      @empty
        <p class="text-gray-600 text-sm">No hay posts relacionados disponibles.</p>
      @endforelse
    </div>

    <div class="bg-white rounded-2xl border p-6">
      <h3 class="font-bold text-lg mb-4">Compartir</h3>
      <div class="flex space-x-3">
        <a href="https://twitter.com/intent/tweet?url={{ urlencode(url()->current()) }}&text={{ urlencode($post->title) }}"
           target="_blank"
           class="bg-blue-500 text-white p-2 rounded-lg hover:bg-blue-600 transition-colors">
          <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
            <path d="M23.953 4.57a10 10 0 01-2.825.775 4.958 4.958 0 002.163-2.723c-.951.555-2.005.959-3.127 1.184a4.92 4.92 0 00-8.384 4.482C7.69 8.095 4.067 6.13 1.64 3.162a4.822 4.822 0 00-.666 2.475c0 1.71.87 3.213 2.188 4.096a4.904 4.904 0 01-2.228-.616v.06a4.923 4.923 0 003.946 4.827 4.996 4.996 0 01-2.212.085 4.936 4.936 0 004.604 3.417 9.867 9.867 0 01-6.102 2.105c-.39 0-.779-.023-1.17-.067a13.995 13.995 0 007.557 2.209c9.053 0 13.998-7.496 13.998-13.985 0-.21 0-.42-.015-.63A9.935 9.935 0 0024 4.59z"/>
          </svg>
        </a>
        
        <a href="https://www.facebook.com/sharer/sharer.php?u={{ urlencode(url()->current()) }}"
           target="_blank"
           class="bg-blue-600 text-white p-2 rounded-lg hover:bg-blue-700 transition-colors">
          <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
            <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/>
          </svg>
        </a>
        
        <a href="https://wa.me/?text={{ urlencode($post->title . ' - ' . url()->current()) }}"
           target="_blank"
           class="bg-green-500 text-white p-2 rounded-lg hover:bg-green-600 transition-colors">
          <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
            <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893A11.821 11.821 0 0020.885 3.488"/>
          </svg>
        </a>
      </div>
    </div>

    <div class="bg-blue-50 border border-blue-200 rounded-2xl p-6">
      <h3 class="font-bold text-lg mb-3 text-blue-900">¿Necesitas más información?</h3>
      <p class="text-blue-800 text-sm mb-4">
        Visita nuestro sitio principal para servicios de ingeniería y consultoría.
      </p>
      <a href="https://www.panoramaingenieria.com" 
         class="inline-block bg-blue-600 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-blue-700 transition-colors">
        Panorama Ingeniería →
      </a>
    </div>
  </aside>
@endsection

@push('head')
<style>
.prose h2 { @apply text-xl font-bold mt-6 mb-4 text-gray-900; }
.prose h3 { @apply text-lg font-semibold mt-5 mb-3 text-gray-900; }
.prose p { @apply mb-4 leading-relaxed; }
.prose ul, .prose ol { @apply mb-4 pl-6; }
.prose li { @apply mb-2; }
.prose a { @apply text-blue-600 hover:text-blue-800 underline; }
</style>
@endpush
